...
# -*- coding: utf-8 -*-
"""
Differential-Fourier-Domain-Regularized-Inversion (example).
...
m is an odd intteger defining the number of pixel areas used to
evaluate the zeroth spatial frequency of the image
...
μ = 0.5
ϵ = 1e-7
CR = 0.03  # Compression ratio. For 256x256 images, CR=0.03 requires 8GB RAM,
# CR=0.1 requires 32GB RAM, for larger dimensions use >=128GB
dim = (256, 256)
# save the matrices to a mat-file
fname = f'd_fdri_cr{round(CR*100)}proc_m{m}_p{p}_mu{μ}_eps{ϵ}.mat'

print('DFDRI EXAMPLE PROGRAM\n')
# initialize the object and set the deafult parameters
...
if os.path.isfile(fname):  # check if the matrices are already precalculated
    ...
    # calculate the continuous low-frequency DCT patterns
    M_dct, SM = dfdri.dct_sampling_functions(CR=CR)
    # find an auxilliary matrix A_m^p,  see sect. 2.1 of ref. [1]
    A = dfdri.auxiliary_matrix_a(m=m, p=p)
    # binarize the DCT patterns, and combine all the sampling patterns into
    # the binary measurement matrix M (see sect. 2.1 of [1] and figs. 1-4)
    M = dfdri.binary_measurement_matrix(M_dct, A)
    # calculate the reconstruction matrix using Eq. (6) of Ref. [1]
    # (takes a lot of time)
    Pg = dfdri.d_fdri(Mbin=M, p=p, μ=μ, ϵ=ϵ)
    print('Saving the measurement and reconstruction matrices to a mat-file\
          (which may be e.g. read in Matlab)')
    io.savemat(fname, {'M': M, 'P': Pg, 'p': p, 'm': m,
               'eps': ϵ, 'mu': μ}, do_compression=True, appendmat=True)
    print('Done')
# create an image  reconstruction function (Eq. (11) of ref. [1] for l=1)
Rec1 = dfdri.reconstruct(Pg, channel=1)
# Rec2=dfdri.reconstruct(Pg,channel=2)
# for a complementary measurement (Eq. (11) of ref. [1] for l=2)
...
print('In order to show that the reconstruction result is invariant to')
print('an additive bias (such as a stable background illumination or the dark')
print('current of the detector, a random constant  is added to')
print(' the measurment vector.')
...
for testnr in range(len(tst_images)):
    ...
    x = np.array(Image.open(fname).resize(size=dim),
                 dtype=np.single).reshape((-1, 1))/255
    print(f'2. Taking the compressive measurement {testnr+1}...\n')

    y = M@x  # This line of code simulates the compressive measurement
    # an added random bias shows that the reconstruction result
    # does not depend on the detector offset
    y += np.single(np.random.rand()*1000)
    # and that the DAQ may use AC-coupling

    print('2. Reconstructing the image...\n')
    t0 = datetime.datetime.now()
    # This line of code reconstructs the image from the compressive
    # measurement y[testnr] using Eq. (11)
    x0 = Rec1(y)
